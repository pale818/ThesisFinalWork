...
OUTPUT_FILE = os.path.join(PROJECT_ROOT, 'pilot_task_ready.csv')


#  LOAD METADATA AND STIMULI
...
#  subjects with existing folders
...
#  STRATIFIED PILOT SAMPLING (10 subjects)

print("\n--- Step 2: Stratified Pilot Sampling ---")
short_sleep = available_df[available_df['Sleep'] <= 6.0]
medium_sleep = available_df[(available_df['Sleep'] > 6.0) & (available_df['Sleep'] < 8.0)]
long_sleep = available_df[available_df['Sleep'] >= 8.0]

pilot_short = short_sleep.sample(n=3, random_state=1)
pilot_med = medium_sleep.sample(n=4, random_state=1)
pilot_long = long_sleep.sample(n=3, random_state=1)

pilot_subjects = pd.concat([pilot_short, pilot_med, pilot_long])
print(f"Pilot group: 3 Short (<=6h), 4 Med (6-8h), 3 Long (>=8h)")
print(f"Selected IDs: {list(pilot_subjects['ID'].astype(str))}")
...
print("\n--- Step 3: Processing Pilot Subjects ---")

for index, row in pilot_subjects.iterrows():
    ...
    #  key columns to analyze (FFT/Power and Performance Metrics)
    ...
    # Parse MarkerIndex for inline event sync (positive = start, negative = end)
    ...
    for marker_id, task_type in id_to_task.items():
        ...
        #  only allowed tasks from stimuli.csv
        if task_clean not in allowed_tasks:
            continue

        # Synchronize wth inline MarkerIndex events in data.csv
        # Positive index = task start, negative = task end
...
if all_task_features:
    ...
    print(f"Total pilot subjects sampled: {len(pilot_subjects)}")
else:
